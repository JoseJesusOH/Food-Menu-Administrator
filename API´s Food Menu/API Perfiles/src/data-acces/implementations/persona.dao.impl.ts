...
export class PersonaDAO implements PersonaIDAO {
    ...
    async eliminarPersonaById(personaId: Number): Promise<Boolean> {
        LoggerAPI.info("Se inicia el metodo para eliminar persona por ID en PersonaDAO")
        try {
            let result = await this.personaRepositorio.delete({ personaId: Number(personaId) });
            if (result.affected && result.affected > 0) {
                LoggerAPI.info("Se elimino la persona en el sistema")
                return true;
            }
            else {
                LoggerAPI.warn("No se elimino la persona en el sistema")
                return false;
            }
        } catch (error) {
            LoggerAPI.error("Error en la eliminacion de persona por ID en el sistema: " + error)
            throw error;
        }
    }

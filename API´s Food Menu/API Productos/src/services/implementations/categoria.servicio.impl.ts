...
class CategoriaServicio implements CategoriaIServicio {
     ...
     */
    async eliminarCategoria(categoriaUuid: String): Promise<Boolean> {
        LoggerAPI.info("Se inicia el metodo para eliminar una nueva categoria");
        try {
            let categoria: Categoria = await this.categoriaDao.getCategoriaByUuid(categoriaUuid);
            if (!categoria) {
                LoggerAPI.warn(`No se ha encontrado la categoria con el UUID: ${categoriaUuid}`);
                return false;
            }
            let resultado = await this.categoriaDao.eliminarCategoriaById(categoria.categoriaId);
            if (resultado) {
                LoggerAPI.info(`Se ha eliminado una nueva categoria con exito`);
                return true;
            }
            else {
                LoggerAPI.warn(`No se ha podido eliminar la nueva categoria`);
                return false;
            }
        } catch (error) {
            LoggerAPI.error(`Error al eliminar una nueva categoria: ${error}`);
            throw error;
        }
    }
